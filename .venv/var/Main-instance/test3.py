...
def check_pending_payment():
    ...
    if booking:
        return jsonify({"redirect_url": url_for('pay_token', booking_id=booking.id)})
...
def waiting_for_payment(booking_id):
    ...
    remaining_seconds = max(0, int((booking.expires_at - datetime.utcnow()).total_seconds()))
    return render_template("waiting_payment.html", booking=booking, remaining=remaining_seconds)
