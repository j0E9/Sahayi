...
def respond_notification(notification_id):
    ...
    response = request.form.get('response', '').capitalize() if not request.is_json else request.json.get('response',
                                                                                                          '').capitalize()
...
@app.route('/check_payment_popup')
@login_required
def check_payment_popup():
    user_id = current_user.id
    bookings = Booking.query.filter_by(status='Token Paid').all()

    for booking in bookings:
        if booking.worker_id == user_id and not booking.popup_shown_to_worker:
            booking.popup_shown_to_worker = True
            db.session.commit()

            giver = booking.provider
            lat, lon = giver.latitude, giver.longitude
        ...
        if booking.provider_id == user_id and not booking.popup_shown_to_provider:
            booking.popup_shown_to_provider = True
            db.session.commit()

            worker = booking.worker
            lat, lon = worker.latitude, worker.longitude
...
def chat(booking_id):
    ...
    return render_template("chat.html", booking_id=booking_id)
